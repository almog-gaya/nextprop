...
interface FilterModalProps {
  ...
  onSaveSmartList: (smartList: { listName: string; filterSpec: any }) => Promise<SmartList | null>;
}
...
const FilterModal: React.FC<FilterModalProps> = ({ isOpen, onClose, onSuccess, onSaveSmartList }) => {
  ...
  const [filterGroups, setFilterGroups] = useState<FilterGroup[]>([{ id: "1", operator: "AND", filters: [] }]);
  ...
  const handleApply = () => {
    if (activeFilter) {
      ...
      // Update filterGroups and then call onSuccess with the updated state
      setFilterGroups((prevGroups) => {
        const updatedGroups = prevGroups.map((group) =>
          group.id === activeGroupId ? { ...group, filters: [...group.filters, newFilter] } : group
        );
        ...
        if (onSuccess) {
          const payload = {
            filters: [
              {
                group: "OR",
                filters: updatedGroups.map((group) => ({
  ...
  const handleSaveSmartList = async (listName: string) => {
    if (!listName || listName.trim() === "") {
      alert("Please enter a valid name for the smart list.");
      return;
    }
  ...
  const handleEditFilter = (groupId: string, filterIndex: number) => {
    ...
    setFilterValue(
      Array.isArray(filter.selectedOption.firstValue) ? filter.selectedOption.firstValue[0].key : filter.selectedOption.firstValue
    );
  ...
  const handleDeleteFilter = (groupId: string, filterIndex: number) => {
    setFilterGroups((prevGroups) => {
      const updatedGroups = prevGroups.map((group) =>
        group.id === groupId ? { ...group, filters: group.filters.filter((_, idx) => idx !== filterIndex) } : group
      );
  ...
  const toggleFilterGroupOperator = (groupId: string, operator: "AND" | "OR") => {
    setFilterGroups((prevGroups) => prevGroups.map((group) => (group.id === groupId ? { ...group, operator } : group)));
        ...
        {filterGroups.map((group, groupIndex) => (
          <div key={group.id} className="mb-6">
            <div className="flex items-center justify-between mb-2">
              <div className="flex gap-2">
                <button
                  ...
                  className={`flex items-center gap-1 px-3 py-1 border rounded text-sm ${
                    group.operator === "AND" ? "bg-blue-50 border-blue-200 text-blue-700" : "border-gray-200 text-gray-700 hover:bg-gray-50"
                  }`}
                >
                  AND
                </button>
                <button
                  ...
                  className={`flex items-center gap-1 px-3 py-1 border rounded text-sm ${
                    group.operator === "OR" ? "bg-blue-50 border-blue-200 text-blue-700" : "border-gray-200 text-gray-700 hover:bg-gray-50"
                  }`}
                >
                  OR
                </button>
              ...
              {groupIndex > 0 && (
                <button onClick={() => removeFilterGroup(group.id)} className="text-red-500 hover:text-red-700">
                  Remove Group
                </button>
              )}
            </div>
              ...
              <div key={idx} className="bg-white border border-gray-200 rounded-lg p-4 mb-4 flex items-center justify-between">
                <div>
                  ...
                  <span className="ml-1 text-sm text-gray-700">
                    {filter.selectedOption.condition === "is_empty"
                      ? "Is empty"
                      : filter.selectedOption.condition === "is_not_empty"
                        ? "Is not empty"
    ...
    return (
      <div className="p-6">
        ...
        <div className="space-y-4">
          <div>
            <div className="flex items-center space-x-2">
              <input type="radio" name="filterOperator" checked={selectedOperator === "is"} onChange={() => setSelectedOperator("is")} />
            ...
            <p className="text-xs text-gray-500 m-2">
              Matches entries based on the exact whole word or phrase specified. Example: If you want to search for 'Field Value', you can
        ...
        <div className="flex justify-end mt-6">
          <button className="px-4 py-2 bg-gray-100 rounded mr-2" onClick={onClose}>
            Cancel
          </button>
          <button className="px-4 py-2 bg-blue-600 text-white rounded" onClick={handleApply}>
            Apply
          </button>
        </div>
      </div>
  ...
  const filteredFields = filterFields.filter((field) => field.label.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  return (
    <div className="fixed inset-0 z-50">
      ...
      {/* Drawer */}
      <div
        className="fixed right-0 top-0 h-full w-full max-w-sm bg-white shadow-2xl flex flex-col transition-transform duration-300"
        style={{ minWidth: 360 }}
      >
