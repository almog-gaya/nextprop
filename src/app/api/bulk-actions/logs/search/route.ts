...
export async function POST(request: Request) {
    const url = 'https://services.leadconnectorhq.com/bulk-actions/logs/search';
    const tokenId = (await refreshTokenIdBackend()).id_token;
    const {locationId} = await getAuthHeaders();
    const {requestId, limit, page, filter} = await request.json();

    console.log(`requestId: ${requestId}, limit: ${limit}, page: ${page}, filter: ${filter}`)
    const payload = {
